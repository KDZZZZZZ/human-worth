...
公网已提供 Go Identity（Google 登录、会话、MCP 凭据）与 Swagger，后端运行在本机 kind lab。2026-09-19 用户已确认真人 Google 登录成功并看到账号。投稿、投票、审核和云端挑战尚未实现。
...
本地文档地址为 `http://127.0.0.1:18191/docs/`。接口筛选、schema 和实现状态可展开查看；禁用 Try it out，不执行 API 请求。0.5.0 契约包含已上线的 Identity 操作。发布步骤见[静态文档部署](docs/deployment.md#swagger-docs)。
